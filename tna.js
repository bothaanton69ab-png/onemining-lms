...
function renderComplianceReport(){
  ...
  h+='<div style="margin-bottom:8px"><b style="font-size:.82rem">SITES</b> <span style="color:#6B7280;font-size:.76rem">(none ticked = all)</span><br>';
  sites.forEach(function(s){ h+='<label style="margin-right:14px;font-size:.85rem"><input type="checkbox" class="cr-site" value="'+s.replace(/"/g,'&quot;')+'"'+(crSites.indexOf(s)>=0?' checked':'')+' onchange="crApply()"> '+s+'</label>'; });
  h+='</div>';
  h+='<div style="margin-bottom:8px"><b style="font-size:.82rem">DEPARTMENTS</b> <span style="color:#6B7280;font-size:.76rem">(none = all)</span><br>';
  depts.forEach(function(d){ h+='<label style="margin-right:14px;font-size:.85rem"><input type="checkbox" class="cr-dept" value="'+d.replace(/"/g,'&quot;')+'"'+(crDepts.indexOf(d)>=0?' checked':'')+' onchange="crApply()"> '+d+'</label>'; });
  h+='</div>';
  h+='<div style="margin-bottom:10px"><b style="font-size:.82rem">STATUS</b> <span style="color:#6B7280;font-size:.76rem">(none = all)</span><br>';
  stats.forEach(function(p){ h+='<label style="margin-right:14px;font-size:.85rem"><input type="checkbox" class="cr-stat" value="'+p[0]+'"'+(crStatuses.indexOf(p[0])>=0?' checked':'')+' onchange="crApply()"> '+p[1]+'</label>'; });
  h+='</div>';
  h+='<div style="display:flex;gap:10px;flex-wrap:wrap"><button class="btn btn-p" style="width:auto" onclick="printCompliance()">🖨 Print / Save as PDF</button><button class="btn btn-o" style="width:auto" onclick="crSites=[];crDepts=[];crStatuses=[];render()">Clear filters</button></div>';
  h+='</div></div>';
