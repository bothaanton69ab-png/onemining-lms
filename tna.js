...
function renderComplianceReport(){
  ...
  list.forEach(function(r){ var e=r.e,c=r.c;
    h+='<tr><td style="font-weight:700;color:#FBB227">'+e.id+'</td><td>'+e.name+'</td><td style="font-size:.75rem">'+(e.contractor?bg('Contractor','gold')+(e.coName?' '+e.coName:'')+(e.coName&&typeof contractorCoApproved==='function'?(contractorCoApproved(e.coName)?' '+bg('Pack ✓','green'):' '+bg('Pack pending','red')):'')+((e.respMgr&&getManagerById(e.respMgr))?'<br>Mgr: '+getManagerById(e.respMgr).name:''):bg('Employee','blue'))+'</td><td style="font-size:.8rem">'+e.site+'</td><td style="font-size:.8rem">'+(e.dept||'-')+'</td><td style="font-size:.76rem">'+(empJobTitle(e.id)||'-')+'</td><td>'+c.valid+'/'+c.req+'</td><td>'+classBadge(r.cls)+'</td></tr>';
  });
