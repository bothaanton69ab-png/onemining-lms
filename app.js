...
sb+='<div class="ni'+(page==='depts'?' a':'')+'" onclick="goPage(\'depts\')">🏢 Departments & Disciplines</div>';
...
else if(page==='depts'&&isA)mc+=renderDepartments();
...
h+='<div class="fg"><label>Department</label><select id="nemp-dept" onchange="emDeptChange()"><option value="">Select...</option>'+((typeof deptNames==="function")?deptNames():[]).map(function(d){return '<option>'+d+'</option>';}).join('')+'</select></div>';
h+='<div class="fg"><label>Discipline</label><select id="nemp-disc"><option value="">Select...</option></select></div>';
...
// Employee management
function toggleAddEmp(){var el=document.getElementById('add-emp-form');if(el){el.classList.toggle('hide');if(!el.classList.contains('hide')){document.getElementById('edit-emp-id').value='';document.getElementById('emp-form-title').textContent='Add Employee';['nemp-id','nemp-idn','nemp-name','nemp-dept'].forEach(function(id){document.getElementById(id).value=''});var _dd=document.getElementById('nemp-disc');if(_dd)_dd.innerHTML='<option value="">Select...</option>';document.getElementById('nemp-gender').value='';document.getElementById('nemp-site').value='';document.getElementById('nemp-pin').value='1234';}}
...
function emDeptChange(keep){ var d=((document.getElementById('nemp-dept')||{}).value)||''; var sel=document.getElementById('nemp-disc'); if(!sel)return; var ds=(typeof disciplinesFor==='function')?disciplinesFor(d):[]; sel.innerHTML='<option value="">Select...</option>'+ds.map(function(x){return '<option>'+x+'</option>';}).join(''); if(keep){ if(ds.indexOf(keep)<0)sel.insertAdjacentHTML('beforeend','<option>'+keep+'</option>'); sel.value=keep; } }
...
var dept=document.getElementById('nemp-dept').value.trim();var disc=((document.getElementById('nemp-disc')||{}).value||'').trim();var site=document.getElementById('nemp-site').value;var pin=document.getElementById('nemp-pin').value.trim()||'1234';var _pt=(document.getElementById('nemp-type')||{}).value||'employee';var _isCon=_pt==='contractor';var _co=_isCon?((document.getElementById('nemp-co')||{}).value||''):'';var _area=_isCon?((document.getElementById('nemp-area')||{}).value||''):'';var _mgr=_isCon?((document.getElementById('nemp-mgr')||{}).value||''):'';
...
if(editId){var idx=emps.findIndex(function(e){return e.id===editId});if(idx>=0)emps[idx]=Object.assign({},emps[idx],{id:eid,idn:idn,name:name,gender:gender,dept:dept,discipline:disc,site:site,pin:pin,contractor:_isCon,coName:_co,area:_area,respMgr:_mgr});}
else{if(emps.some(function(e){return e.id.toUpperCase()===eid.toUpperCase()})){alert('Already exists');return}emps.push({id:eid,idn:idn,name:name,gender:gender,dept:dept,discipline:disc,site:site,pin:pin,contractor:_isCon,coName:_co,area:_area,respMgr:_mgr});}
save();render();}
...
document.getElementById('nemp-gender').value=e.gender;var _dsel=document.getElementById('nemp-dept');if(_dsel){if(e.dept&&!Array.prototype.some.call(_dsel.options,function(o){return o.value===e.dept;}))_dsel.insertAdjacentHTML('beforeend','<option>'+e.dept+'</option>');_dsel.value=e.dept||'';}emDeptChange(e.discipline||'');document.getElementById('nemp-site').value=e.site;document.getElementById('nemp-pin').value=e.pin;var _t=document.getElementById('nemp-type');if(_t)_t.value=e.contractor?'contractor':'employee';var _cf=document.getElementById('con-fields');if(_cf)_cf.style.display=e.contractor?'contents':'none';var _c2=document.getElementById('nemp-co');if(_c2)_c2.value=e.coName||'';var _a2=document.getElementById('nemp-area');if(_a2)_a2.value=e.area||'';var _m2=document.getElementById('nemp-mgr');if(_m2)_m2.value=e.respMgr||'';},50);}
...
h+='<div class="fg"><label>Department</label><select id="ce-dept" onchange="ceDeptChange()"><option value="">— none —</option>'+((typeof deptNames==='function')?deptNames():[]).map(function(d){return '<option'+(s.dept===d?' selected':'')+'>'+d+'</option>';}).join('')+'</select></div>';
h+='<div class="fg"><label>Discipline</label><select id="ce-disc"><option value="">— none —</option>'+((typeof disciplinesFor==='function')?disciplinesFor(s.dept||''):[]).map(function(x){return '<option'+(s.discipline===x?' selected':'')+'>'+x+'</option>';}).join('')+((s.discipline&&(typeof disciplinesFor==='function')&&disciplinesFor(s.dept||'').indexOf(s.discipline)<0)?'<option selected>'+s.discipline+'</option>':'')+'</select></div>';
...
function renderContractorCos(){
  ...
  h+='<div class="card"><div class="tw"><table><thead><tr><th>Company</th><th>People</th><th>Pack status</th><th>Documents</th><th>Signed off</th><th>Expiry</th><th>Manage</th></tr></thead><tbody>';
  ...
  rows.forEach(function(c){
    ...
    h+='<td><input type="date" id="cco-sign-'+c.id+'" value="'+(c.signedDate||'')+'" title="Date the contractor pack was signed off" style="padding:6px 8px;border:1.5px solid #e2e5e9;border-radius:6px;font-size:.8rem"></td>';
    h+='<td><input type="date" id="cco-exp-'+c.id+'" value="'+(c.expiry||'')+'" title="Expiry date for this pack (set per this contractor\'s rules)" style="padding:6px 8px;border:1.5px solid #e2e5e9;border-radius:6px;font-size:.8rem"></td>';
  ...
  unreg.sort().forEach(function(nm){ var ppl=emps.filter(function(e){return e.contractor&&e.coName===nm;}).length;
    h+='<tr style="background:#fff8ec"><td style="font-weight:600">'+nm+'</td><td style="font-weight:700">'+ppl+'</td><td>'+bg('No pack set','gray')+'</td><td colspan="3" style="font-size:.78rem;color:#6B7280">Has contractors but no pack set up yet.</td><td><button class="btn btn-p btn-sm" onclick="addCoByName(\''+encodeURIComponent(nm)+'\')">+ Set up pack</button></td></tr>';
  });
  if(!rows.length&&!unreg.length)h+='<tr><td colspan="7" style="text-align:center;color:#6B7280;padding:20px">No contractor companies yet. Add one above, or add a contractor under Manage Employees.</td></tr>';
  h+='</tbody></table></div></div></div>';
  return h;
}
...
function setCoStatus(id,st){ if(typeof can==='function'&&!can('packs')){alert('Only the contractor-pack approver (or admin) can change pack status.');return;} var c=contractorCos.find(function(x){return x.id===id;}); if(!c)return; var exp=document.getElementById('cco-exp-'+id); if(exp)c.expiry=exp.value||''; var sgn=document.getElementById('cco-sign-'+id); if(sgn)c.signedDate=sgn.value||''; if(st==='approved'){ if(!(c.packDocs&&c.packDocs.length)){ if(!confirm('No pack document has been uploaded yet. Approve anyway?'))return; } c.packStatus='approved'; c.approvedBy=(user&&user.name)||'admin'; c.approvedAt=now(); } else { c.packStatus=st; c.approvedAt=''; c.approvedBy=''; } if(typeof logAudit==='function')logAudit('CONTRACTOR PACK '+st.toUpperCase(), c.name, ''); save(); render(); }
...
function uploadCoDoc(id){ if(typeof can==='function'&&!can('packs')){alert('Only the contractor-pack approver (or admin) can upload pack documents.');return;} var inp=document.createElement('input'); inp.type='file'; inp.accept='.pdf,.doc,.docx,image/*'; inp.onchange=async function(e){ var f=e.target.files[0]; if(!f)return; if(f.size>50*1024*1024){alert('That file is '+Math.round(f.size/1048576)+'MB — the limit is 50MB. Please compress or split it.');return;} try{ var path='contractor-packs/'+id+'_'+Date.now()+'_'+_safeName(f.name); var r=await sb.storage.from('lms-files').upload(path,f,{upsert:true}); if(r.error){alert('Upload failed: '+(r.error.message||'unknown error')+'. Please try a PDF and check your connection.');return;} var u=sb.storage.from('lms-files').getPublicUrl(path); var c=contractorCos.find(function(x){return x.id===id;}); if(!c){alert('Company not found — please refresh.');return;} c.packDocs=c.packDocs||[]; c.packDocs.push({url:u.data.publicUrl,name:f.name}); if(typeof logAudit==='function')logAudit('CONTRACTOR PACK DOC UPLOADED', c.name, f.name); await save(); render(); alert('Pack document uploaded.'); }catch(err){ alert('Upload error: '+(err&&err.message?err.message:err)); } }; inp.click(); }
